  ...
  <v-container>
    <v-layout row justify-center>
      <v-flex xs12 md5>
              ...
              <v-container text-xs-right>
                <transition name="button-fade" mode="out-in">
                  ...
                  <v-subheader
                    ...
                    v-else
                    class="green--text">
                    An email has been sent with password rest link
                  </v-subheader>
                </transition>
              </v-container>
            </v-card-actions>
        ...
        <p align="center">
          <a href="mailto:accounts@aiubcommunity.com">
            <code class="support-email grey--text">accounts@aiubcommunity.com</code>
          </a>
        </p>
      </v-flex>
    </v-layout>
        ...
        if (this.form.valid) {
          ...
          .catch((error) => {
            if (error.response.status === 422) {
              var field = error.response.data.field
              var msg = error.response.data.msg
              this.errorMsgs[field] = msg
